...
class BillService {
  ...
  static Future<bool> generateKitchenSlip({
    ...
    pdf.addPage(
      pw.Page(
        pageFormat: _getDynamicThermalFormat(items.length, extraRows: 2),
        theme: theme,
        build: (ctx) => pw.Column(
  ...
  static Future<bool> generateAndDownloadBill({
    ...
    pdf.addPage(
      pw.Page(
        pageFormat: _getDynamicThermalFormat(items.length, extraRows: appliedCharges.length + 5),
        theme: theme,
        build: (ctx) => pw.Column(
          crossAxisAlignment: pw.CrossAxisAlignment.start,
          children: [
  ...
  static Future<bool> generateCombinedSlipAndBill({
    ...
    if (isSilentPrinting) {
      ...
      for (final entry in groupedKitchen.entries) {
        ...
        pdf.addPage(
          pw.Page(
            pageFormat: _getDynamicThermalFormat(stationItems.length, extraRows: 2),
            theme: theme,
      ...
      if (receiptItems.isNotEmpty) {
        ...
        pdf.addPage(
          pw.Page(
            pageFormat: _getDynamicThermalFormat(receiptItems.length, extraRows: appliedCharges.length + 5),
            theme: theme,
            build: (ctx) => pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.start,
  ...
  static Future<void> printReport({
    ...
    pdf.addPage(
      pw.Page(
        pageFormat: _getDynamicThermalFormat(items.length, extraRows: 25),
        theme: theme,
        build: (ctx) => pw.Column(
          crossAxisAlignment: pw.CrossAxisAlignment.start,
          children: [
