...
String formatDate(DateTime dt) {
  if (CalendarSettings.currentType == CalendarType.ethiopian) {
    final et = dt.asETDateTime;
    return ETDateFormat("MMM d, yyyy HH:mm").format(et);
  }
...
class EnhancedPrintService {
    ...
    pdf.addPage(
      pw.Page(
        pageFormat: PdfPageFormat(
          _paperWidth * PdfPageFormat.mm,
          _paperHeight * PdfPageFormat.mm,
        ),
  ...
  static Future<void> printFinalReceipt({
    ...
    await Printing.layoutPdf(
      onLayout: (PdfPageFormat format) async => pdf.save(),
      name:
          'Final_Receipt_${combinedOrder.tableName}_${now.millisecondsSinceEpoch}',
    );
  }
  ...
  static pw.Widget _buildKitchenHeader(
    ...
    return pw.Column(
      children: [
        ...
        pw.SizedBox(height: 5),
        pw.Text(formatDate(now), style: const pw.TextStyle(fontSize: 10)),
        pw.Divider(thickness: 1),
      ],
    );
  }

  static pw.Widget _buildKitchenOrderInfo(
    OrderModel order,
    String Function(String) t,
  ) {
    return pw.Container(
  ...
  static pw.Widget _buildKitchenItems(
  ...
  static pw.Widget _buildKitchenFooter(
    String Function(String) t,
    DateTime now,
  ) {
    return pw.Column(
  ...
  static pw.Widget _buildReceiptOrderInfo(
  ...
  static String _numberToWords(double amount, String Function(String) t) {
    ...
    if (decimalPart > 0) {
      words +=
          ' ${t('numbers.and')} ${_convertNumberToWords(decimalPart, t)} ${t('numbers.cents')}';
    }
